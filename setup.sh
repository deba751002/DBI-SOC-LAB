...
if [[ ! -f .env ]]; then
  ...
  sed -i "s/IRIS_ADMIN_PASSWORD=.*/IRIS_ADMIN_PASSWORD=$(openssl rand -hex 12)/" .env
  sed -i "s/IRIS_DB_PASSWORD=.*/IRIS_DB_PASSWORD=$(openssl rand -hex 12)/" .env
  sed -i "s/MISP_ADMIN_PASSWORD=.*/MISP_ADMIN_PASSWORD=$(openssl rand -hex 12)/" .env
  sed -i "s/MISP_DB_PASSWORD=.*/MISP_DB_PASSWORD=$(openssl rand -hex 12)/" .env
  sed -i "s/ST2_AUTH_TOKEN=.*/ST2_AUTH_TOKEN=$(openssl rand -hex 20)/" .env
  sed -i "s/CALDERA_API_KEY_RED=.*/CALDERA_API_KEY_RED=$(openssl rand -hex 16)/" .env
  sed -i "s/CALDERA_API_KEY_BLUE=.*/CALDERA_API_KEY_BLUE=$(openssl rand -hex 16)/" .env

  ok ".env created with generated credentials"
  warn "Review .env and set CALDERA_RED_PASS, CALDERA_BLUE_PASS before use"
else
...
step "Velociraptor configuration"
if [[ ! -f config/velociraptor/server.config.yaml ]]; then
  info "Generating Velociraptor server + client config..."
  mkdir -p config/velociraptor
  docker run --rm -v "$(pwd)/config/velociraptor:/etc/velociraptor" \
    wlambert/velociraptor:latest config generate > config/velociraptor/server.config.yaml
  docker run --rm -v "$(pwd)/config/velociraptor:/etc/velociraptor" \
    wlambert/velociraptor:latest config client_config \
    --config /etc/velociraptor/server.config.yaml \
    > config/velociraptor/client.config.yaml
  ok "Velociraptor config generated"
else
  ok "Velociraptor config already exists"
fi
...
curl -sk -X PUT \
  -u "admin:$(grep OPENSEARCH_INITIAL_ADMIN_PASSWORD .env | cut -d= -f2)" \
...
docker exec soc-ollama ollama pull llama3.2:3b 2>/dev/null || warn "Ollama model pull failed — run manually: docker exec soc-ollama ollama pull llama3.2:3b"
