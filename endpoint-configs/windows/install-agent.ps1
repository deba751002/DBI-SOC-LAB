...
Write-Host "   [OK] Winlogbeat installed and started" -ForegroundColor Green
...
Write-Host " Sysmon:     $(if ($sysmonSvc.Status -eq 'Running') {'[OK] Running'} else {'[FAIL] NOT Running'})" -ForegroundColor $(if ($sysmonSvc.Status -eq 'Running') {'Green'} else {'Red'})
Write-Host " Winlogbeat: $(if ($wlbSvc.Status -eq 'Running') {'[OK] Running'} else {'[FAIL] NOT Running'})" -ForegroundColor $(if ($wlbSvc.Status -eq 'Running') {'Green'} else {'Red'})
